...
#+BEGIN_COMMENT
Massive performance boost once we found out and fixed textures that were imported at extremely high resolutions.
Since full resolution will be used when we export we chose to reduce the resolutions significantly for the
real-time generation and rendering.

Progress on Methods chapters.
A better approx rectangle for polygons with 4 vertices.
Road user input
#+END_COMMENT
...
Terrain size can now be changed from the GUI
Restructure of Methods chapter due to feedback from fackspråk and the first pages of this chapter are almost done.
#+END_COMMENT
